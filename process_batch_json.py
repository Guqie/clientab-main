"""Process JSON batch files for 战新与未来产业月报 - clean content and write summaries."""
import asyncio
import aiohttp
import json
import os
...
PRO_MODEL = "deepseek-chat"  # Use flash for both since pro/reasoner might be slower

BATCH_DIR = Path(r"d:\桌面\clientab-main\temp-data\json_batch")
FILES_TO_PROCESS = [f"item_{i:03d}.json" for i in range(24, 36)]  # item_024 to item_035
...
- 数字、单位、英文缩写按原文保留；不确定的金额、产能、指标不要改写；
- 不输出"信息来源"，该字段由 Word 生成器自动生成。'''


async def call_deepseek(session, prompt, text, max_tokens=800):
    """Call DeepSeek API for a single request."""
...
async def process_file(session, filepath):
    """Process a single JSON file: clean content and write summary."""
    ...
    if not content or len(content.strip()) < 50:
        print(f"    Skipping {filepath.name} - content too short", flush=True)
        data["content_cleaned"] = content
        data["summary"] = ""
        data["summary_status"] = "skipped_empty"
    ...
    try:
        # Step 1: Clean the content (flash cleaning)
        content_cleaned = await call_deepseek(
            session, FLASH_CLEANING_PROMPT, content, max_tokens=1500
        )
        
        # Step 2: Write summary (pro summarization)
        # Include category info for better summarization
        heading_1 = data.get("heading_1", "")
        heading_2 = data.get("heading_2", "")
        title = data.get("title", "")
        
        summary_prompt = f'''【一级栏目】{heading_1}
【二级栏目】{heading_2}
【标题】{title}

{PRO_SUMMARIZATION_PROMPT}

【清洗后正文】
{content_cleaned}'''
        
        summary = await call_deepseek(
            session, summary_prompt, content_cleaned, max_tokens=1500
        )
        
        # Update data
        data["content_cleaned"] = content_cleaned.strip()
        ...
        print(f"    ✓ Success - content_cleaned: {len(content_cleaned)} chars, summary: {len(summary)} chars", flush=True)
        return "success"
...
async def main():
    print("=" * 60)
    print("战新与未来产业月报 - JSON Batch Processor")
    print("=" * 60)
    ...
    async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=120)) as session:
        for filename in FILES_TO_PROCESS:
            ...
            result = await process_file(session, filepath)

...
- 位置参数 `csv_file`：单文件模式下的 CSV 路径，可与 `--interactive` 互斥（详见验证逻辑）。
- 批量处理：`--batch-dir` 指定目录，`--batch-pattern` 文件匹配模式（默认 `*.csv`），`--max-workers` 并发线程数。
- 输出格式：`--format {docx,pdf,html,markdown,excel,json}`，默认 `docx`。
- 异步处理：`--async` 启用异步；`--progress` 显示进度条。
- 模板与校验：`--list-templates` 列出模板，`--validate-only` 仅验证 CSV，不生成文档。
- 日志与输出：`-v/--verbose` 详细输出，`-q/--quiet` 静默输出，`--log-level {DEBUG,INFO,WARNING,ERROR,CRITICAL}`。
- 其他：`--output` 指定目标文件路径，`--output-dir` 指定输出目录，`--image-timeout` 图片下载超时（秒），`--max-retries` 图片下载重试次数，`--no-images` 禁用图片下载，`--port` 启动 Web 服务端口。
...
# 仅验证 CSV（不生成）
python -m csv_word_converter .\data\sample.csv --validate-only;

# 批量模式（并发 8；导出 PDF）
python -m csv_word_converter --batch-dir .\data --batch-pattern *.csv --max-workers 8 --format pdf --progress;

# 异步单文件（展示进度）
python -m csv_word_converter .\data\sample.csv -t technology --async --progress;
```
...
```python
from csv_word_converter import get_available_templates, validate_csv_file

templates = get_available_templates()
print("可用模板:", templates)

info = validate_csv_file("data.csv")
print("CSV校验:", info)
```
...
```python
import asyncio
from src.csv_word_converter.async_converter import AsyncConverter

async def main():
    converter = AsyncConverter(max_workers=2, max_concurrent_tasks=4)
    task_id = await converter.add_task(
        csv_file=Path("data.csv"),
        output_path=Path("outputs/data.docx")
    )
...
- 批量模式未生效：确认 `--batch-dir` 指向存在的目录且包含匹配模式的文件；检查 `--batch-pattern` 与大小写。
- 进度条不显示：需显式传入 `--progress`；交互模式中会提示开关。
...
# 设置远程（将URL替换为你的仓库地址）
git branch -M main;
git remote remove origin; git remote add origin https://github.com/<your-username>/clientab-main.git;
